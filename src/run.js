 ...
 */
async function runWorkload(workload, backend, inputSize, executor) {
  let originScoresArray = [];
  let scoresArray = [];
  const flags = settings.chrome_flags;
  for (let i = 0; i < workload.run_times; i++) {
    let thisScore = await executor(workload, backend, inputSize, flags);
...
async function genWorkloadResult(
    deviceInfo, workLoadName, workload, backend, inputSize, executor,
    timestamp) {
  // if (!settings.dev_mode) {
  //   await syncRemoteDirectory(workload, 'pull');
  // }
  let results = await runWorkload(workload, backend, inputSize, executor);
  let jsonData = {
...
async function genWorkloadsResults(deviceInfo, target, timestamp) {
  ...
  for (let i = 0; i < workloads_length; i++) {
    ...
    console.log(workload.inputSize);

    for (var j = 0; j < workload.inputSize.length; j++) {
      const inputSize = workload.inputSize[j];
      workload.url = 'http://wp-27.sh.intel.com/workspace/server/workspace/project/tfjswebgpu/tfjs/e2e/benchmarks/local-benchmark/';

      for (var k = 0; k < workload.backends.length; k++) {
        const backend = workload.backends[k];
        var warmupRuns = '?warmup=2&run=2';
        if (inputSize === 1024 || inputSize === 2.0)
          warmupRuns = '?warmup=1&run=1';
        workload.url = workload.url + warmupRuns;
        workLoadName =
            backend + '_' + workload.name + '_' + inputSize.toString();
        results[workLoadName] = await genWorkloadResult(
            deviceInfo, workLoadName, workload, backend, inputSize, runTFJS,
            timestamp);
      }
    }
  }
